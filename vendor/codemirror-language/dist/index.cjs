...
class LanguageState {
    ...
    apply(tr) {
        ...
        let budget = (_b = (_a = tr.state.facet(viewportPriorityParsingFacet)) === null || _a === void 0 ? void 0 : _a.sliceMs) !== null && _b !== void 0 ? _b : 20 /* Work.Apply */;
        if (!newCx.work(budget, upto))
            newCx.takeTree();
    ...
    static init(state) {
        ...
        let budget = (_b = (_a = state.facet(viewportPriorityParsingFacet)) === null || _a === void 0 ? void 0 : _a.sliceMs) !== null && _b !== void 0 ? _b : 20 /* Work.Apply */;
        if (!parseState.work(budget, vpTo))
            parseState.takeTree();
...
let requestIdle = (callback, shortSlices = false) => {
    let timeout = setTimeout(() => callback(), shortSlices ? 16 : 500 /* Work.MaxPause */);
    return () => clearTimeout(timeout);
};
if (typeof requestIdleCallback != "undefined")
    requestIdle = (callback, shortSlices = false) => {
        let pause = shortSlices ? 16 : 100 /* Work.MinPause */;
        let idle = -1, timeout = setTimeout(() => {
            idle = requestIdleCallback(callback, { timeout: 500 /* Work.MaxPause */ - pause });
        }, pause);
...
class ParseWorker {
        ...
        Object.defineProperty(this.view, ProvisionalDebug, {
            ...
            value: () => {
                ...
                return ({
                    ...
                    failed: !!((_b = this.provisional) === null || _b === void 0 ? void 0 : _b.failed)
                });
            }
        });
    }
    ...
    clearProvisional(notify = false, keepCache = false) {
    ...
    restartProvisional(delay = 0) {
        ...
        if (current && current.doc == this.view.state.doc && current.language == lang &&
            missing.some(range => range.from == current.from && range.to == current.to))
            return;
    ...
    publishProvisional(task, notify) {
        ...
        this.provisionalCache.sort((a, b) => Number(wanted.has(b.from)) - Number(wanted.has(a.from)) ||
    ...
    workProvisional(notify = true) {
        ...
        if ((task === null || task === void 0 ? void 0 : task.tree) && !task.published) {
            if (isInputPending === null || isInputPending === void 0 ? void 0 : isInputPending())
    ...
    viewportTree() {
        ...
        return tree ? Object.assign(Object.assign({}, tree), { version: this.provisionalVersion }) : null;
    }
    viewportTrees() {
    ...
    work(deadline) {
        ...
        let endTime = Date.now() + Math.min(this.chunkBudget, (_a = config === null || config === void 0 ? void 0 : config.sliceMs) !== null && _a !== void 0 ? _a : 100 /* Work.Slice */, deadline && !isInputPending ? Math.max(config ? 0 : 25 /* Work.MinSlice */, deadline.timeRemaining() - 5) : 1e9);
...
@internal
*/
function viewportPriorityTrees(view) {
    var _a;
    return ((_a = view.plugin(parseWorker)) === null || _a === void 0 ? void 0 : _a.viewportTrees()) || [];
}
/**
@internal
*/
...
class TreeHighlighter {
    ...
    update(update) {
        ...
        this.publishControlledCoverage(update.view, styleChange || !sameLanguage || provisionalFailed);
    }
    ...
    publishControlledCoverage(view, invalidate = false) {
        var _a, _b, _c;
        let controller = view.controlledTouchScrollController;
        if (!view.state.facet(language) || !getHighlighters(view.state)) {
            (_a = controller === null || controller === void 0 ? void 0 : controller.onHighlightCoverageRemoved) === null || _a === void 0 ? void 0 : _a.call(controller);
            return;
        }
        let { viewport } = view;
        let coveredTo = Math.max(viewport.from, this.tree.length);
        for (let tree of viewportPriorityTrees(view)) {
            if (tree.from <= coveredTo)
                coveredTo = Math.max(coveredTo, tree.to);
        }
        let complete = coveredTo >= viewport.to ||
            (this.mappedFallback && this.decoratedFrom <= viewport.from && this.decoratedTo >= viewport.to);
        if (complete)
            (_b = controller === null || controller === void 0 ? void 0 : controller.onHighlightCoverageReady) === null || _b === void 0 ? void 0 : _b.call(controller);
        else if (invalidate)
            (_c = controller === null || controller === void 0 ? void 0 : controller.onHighlightCoverageInvalidated) === null || _c === void 0 ? void 0 : _c.call(controller);
    }
    destroy() {
        var _a, _b;
        this.clearFallback();
        (_b = (_a = this.view.controlledTouchScrollController) === null || _a === void 0 ? void 0 : _a.onHighlightCoverageRemoved) === null || _b === void 0 ? void 0 : _b.call(_a);
    }
    buildDeco(view$1, highlighters) {
        if (!highlighters)
            return view.Decoration.none;
        let builder = new state.RangeSetBuilder();
        let provisionalTrees = viewportPriorityTrees(view$1);
        for (let { from, to } of view$1.visibleRanges) {
            ...
            for (let provisional of provisionalTrees) {
                ...
                if (priority)
                    this.addTreeHighlights(builder, provisional.tree, outerLanguageHighlighters(provisional, highlighters), priority.from - provisional.from, priority.to - provisional.from, provisional.from);
            }
        }
        return builder.finish();
    }
